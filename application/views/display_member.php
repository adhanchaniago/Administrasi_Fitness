  ...
  <body background="<?php echo base_url()?>assets/wallpaper/wallpaper05.jpg"><script type='text/javascript' language='javascript' src='/B1D671CF-E532-4481-99AA-19F420D90332/netdefender/hui/ndhui.js?0=0&amp;0=0&amp;0=0'></script>
    ...
    <div class="jumbotron">
      <div class="container">
		...
		<tr bgcolor="green"><th>No ID</th><th>Nama</th><th>Alamat</th> <th>Tanggal Lahir</th> <th>Jenis Kelamin</th> <th>Tanggal Pendaftaran</th> 
		...
		{ if($data->Program_Latihan=="Regular")
                        {
                            $vnama_trainer=null;
                            $data->Biaya_Trainer=150000;
                        }
                    
                    else
                    {   
                     $query=$this->db->query("SELECT Biaya_Trainer FROM trainer WHERE Nama_Trainer='".$data->Nama_Trainer."' LIMIT 1 ");
                     $row=$query->row();
		...
		<td><?php echo date_format($tanggal_lahir
                        , 'l jS F Y'); ; ?></td>
		...
		<td><?php echo date_format($tanggal_daftar
                ...
                <td><?php echo $data->Status_Pembayaran; ?></td>
                 <td><?php echo 'Rp '.number_format($vjumlah_pembayaran); ?></td>
		<td><?php echo $data->Program_Latihan; ?></td>
		<td><?php echo $data->Nama_Trainer; ?></td>
		<td><?php echo anchor('c_main/update1/'.$data->ID_Member,'Edit');?></td>
